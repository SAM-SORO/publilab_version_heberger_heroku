...
<div id="content" class="p-4 p-md-5 pt-5 mt-4 ">
    ...
    <div class="col-10 mx-auto form-container shadow p-5 mb-5 bg-body rounded">
        <h2 class="mb-5 text-center" style="color: #2a52be;">MODIFIER UN ARTICLE</h2>
        <form action="{{ route('chercheur.enregistrer-modification-article', $article->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method("POST")
            <div class="form-group row mb-4">
                <label for="article-title" class="col-sm-3 col-form-label">Titre de l'article</label>
                <div class="col-sm-9">
                    <input type="text" class="form-control" id="article-title" name="titre" value="{{ $article->titre }}" placeholder="Saisir le titre">
                </div>
            </div>
            ...
            <div class="form-group row mb-4">
                ...
                <div class="col-sm-9">
                    <textarea class="form-control" id="article-description" name="description" rows="3" placeholder="Saisir la description">{{ $article->description }}</textarea>
                </div>
            </div>
            ...
            <div class="text-right">
                <button type="submit" class="btn btn-success submit-button py-2 px-4">Modifier l'article</button>
            </div>
        </form>
    </div>
</div>
